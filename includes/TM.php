...
class TransM {
	...
	static function WGS84ToUTM( &$parser, $phi_d, $lambda_d, $zone = '' ) {
		...
		// Based on http://www.igorexchange.com/node/927 and http://home.hiwaay.net/~taylorc/toolbox/geography/geoutm.html
		$bandletter = [ "C", "D", "E", "F", "G", "H", "J", "K", "L", "N", "P", "Q", "R", "S", "T", "U", "V", "W", "X", "X" ];
		$TMScaleFactor = 0.9996;
		if ( $phi_d >= 84 || $phi_d <= -80 ) {
			return "Polar area, use Universal Polar Stereographic (UPS)";
		}
	...
	static function WGS84ToMTM( &$parser, $phi_d, $lambda_d, $zone = '' ) {
		...
		$mtmSmers = [ 0, -53, -56, -58.5, -61.5, -64.5, -67.5, -70.5, -73.5, -76.5, -79.5, -82.5, -81, -84, -87, -90, -93, -96, -99, -102, -105, -108, -111, -114, -117, -120, -123, -126, -129, -132, -135, -138, -141 ];  // last was 142 ?!! I think it should be 141.
		...
		if ( !$zone ) {  // determine zone from lat/lon
			if ( $lambda_d < -51.5 && $lambda_d >= -54.5 ) {
				$zone = 1;
			}
			if ( $lambda_d < -54.5 && $lambda_d >= -57.5 ) {
				$zone = 2;
			}
			if ( $lambda_d < -57.5 && $lambda_d >= -59.5 && $phi_d <= 46.5
			|| $lambda_d < -57.5 && $lambda_d >= -60 && $phi_d > 46.5 ) {
				$zone = 3;
			}
			if ( $lambda_d < -59.5 && $lambda_d >= -63. && $phi_d < 46.5
			|| $lambda_d < -60 && $lambda_d >= -63 && $phi_d >= 46.5 ) {
				$zone = 4;
			}
			if ( $lambda_d < -63 && $lambda_d >= -66.5 && $phi_d <= 44.75
			|| $lambda_d < -63 && $lambda_d >= -66 && $phi_d > 44.75 ) {
				$zone = 5;
			}
			if ( $lambda_d < -66 && $lambda_d >= -69 && $phi_d > 44.75
			|| $lambda_d < -66.5 && $lambda_d >= -69 && $phi_d <= 44.75 ) {
				$zone = 6;
			}
			if ( $lambda_d < -69 && $lambda_d >= -72 ) {
				$zone = 7;
			}
			if ( $lambda_d < -72 && $lambda_d >= -75 ) {
				$zone = 8;
			}
			if ( $lambda_d < -75 && $lambda_d >= -78 ) {
				$zone = 9;
			}
			if ( $lambda_d < -78 && $lambda_d >= -79.5 && $phi_d > 47
			|| $lambda_d < -78. && $lambda_d >= -80.25 && $phi_d <= 47 && $phi_d > 46
			|| $lambda_d < -78 && $lambda_d >= -81 && $phi_d <= 46 ) {
				$zone = 10;
			}
			if ( $lambda_d < -81 && $lambda_d >= -84 && $phi_d <= 46 ) {
				$zone = 11;
			}
			if ( $lambda_d < -79.5 && $lambda_d >= -82.5 && $phi_d > 47
			|| $lambda_d < -80.25 && $lambda_d >= -82.5 && $phi_d <= 47 && $phi_d > 46 ) {
				$zone = 12;
			}
			if ( $lambda_d < -82.5 && $lambda_d >= -85.5 && $phi_d > 46 ) {
				$zone = 13;
			}
