...
$wgExtensionFunctions[] = 'efCoorconv_ParserFunction_Init';
# Add a hook to initialise the magic word
$wgHooks['LanguageGetMagic'][]       = 'efCoorconv_ParserFunction_Magic';
 
function efCoorconv_ParserFunction_Init() {
        global $wgParser;
        $wgParser->setFunctionHook( 'lat_deg2dms',      'latDegToDMS'  );
        $wgParser->setFunctionHook( 'lat_dms2deg',      'latDMSToDeg'  );
        $wgParser->setFunctionHook( 'long_deg2dms',     'longDegToDMS' );
        $wgParser->setFunctionHook( 'long_dms2deg',     'longDMSToDeg' );
        $wgParser->setFunctionHook( 'wgs84_2rd',        'WGS84ToRD' );
        $wgParser->setFunctionHook( 'wgs84_2lb93',      'WGS84ToLAM93' );
        $wgParser->setFunctionHook( 'wgs84_2lb08',      'WGS84ToLAM08' );
        $wgParser->setFunctionHook( 'wgs84_2ch03',      'WGS84ToCH1903' );
        $wgParser->setFunctionHook( 'lat_long2utm',     'WGS84ToUTM' );
        $wgParser->setFunctionHook( 'wgs84_2itm',       'WGS84ToITM' );
        $wgParser->setFunctionHook( 'wgs84_2tm35fin',   'WGS84ToTM35FIN' );
        $wgParser->setFunctionHook( 'wgs84_2mtm',       'WGS84ToMTM' );
        $wgParser->setFunctionHook( 'wgs84_2osgb',      'WGS84ToOSGB36' );
        $wgParser->setFunctionHook( 'wgs84_2ig',        'WGS84ToIG' );
        $wgParser->setFunctionHook( 'wgs84_2luref',     'WGS84ToLUREF' );
 
        return true;    
}
 
function efCoorconv_ParserFunction_Magic( &$magicWords, $langCode ) {
        $magicWords['lat_deg2dms']      = array( 0, 'lat_deg2dms' );
        $magicWords['lat_dms2deg']      = array( 0, 'lat_dms2deg' );
        $magicWords['long_deg2dms']     = array( 0, 'long_deg2dms' );
        $magicWords['long_dms2deg']     = array( 0, 'long_dms2deg' );
        $magicWords['wgs84_2rd']        = array( 0, 'wgs84_2rd' );
        $magicWords['wgs84_2lb93']      = array( 0, 'wgs84_2lb93' );
        $magicWords['wgs84_2lb08']      = array( 0, 'wgs84_2lb08' );
        $magicWords['wgs84_2ch03']      = array( 0, 'wgs84_2ch03' );
        $magicWords['lat_long2utm']     = array( 0, 'lat_long2utm' );
        $magicWords['wgs84_2itm']       = array( 0, 'wgs84_2itm' );
        $magicWords['wgs84_2tm35fin']   = array( 0, 'wgs84_2tm35fin' );
        $magicWords['wgs84_2mtm']       = array( 0, 'wgs84_2mtm' );
        $magicWords['wgs84_2osgb']      = array( 0, 'wgs84_2osgb' );
        $magicWords['wgs84_2ig']        = array( 0, 'wgs84_2ig' );
        $magicWords['wgs84_2luref']     = array( 0, 'wgs84_2luref' );
 
        return true;
}
 
function latDegToDMS( &$parser, $degrees ) {
        $h = $degrees < 0 ? 'S' : 'N';
...
function latDMSToDeg( &$parser, $d, $m, $s, $h ) {
        $degrees = ($d * 3600 + $m * 60 + $s) / 3600.0 * ($h == 'N' ? 1 : -1);
        return $degrees;
}
 
function longDegToDMS( &$parser, $degrees ) {
        $h = $degrees < 0 ? 'W' : 'E';
...
function longDMSToDeg( &$parser, $d, $m, $s, $h ) {
...
function WGS84ToLAM93( &$parser, $x, $y ) {
...
// based on http://professionnels.ign.fr/DISPLAY/000/526/695/5266956/Geodesie__projections.pdf
        ...
        return WGS84ToLAM($LB93, $x, $y );
}
 
function WGS84ToLAM08( &$parser, $x, $y ) {
...
// based on http://www.ngi.be/Common/Lambert2008/Transformation_Geographic_Lambert_NL.pdf
        ...
        return WGS84ToLAM($LB08, $x, $y );
}
 
function WGS84ToLAM($LAM,$x, $y ) {
// Lambert projection
// based on http://www.ngi.be/Common/Lambert2008/Transformation_Geographic_Lambert_NL.pdf
        extract($LAM);
...
function WGS84ToCH1903( &$parser, $x, $y ) {
...
//      return sprintf("%06d%06d",$x_rd, $y_rd);
        return number_format($x_rd/1000, 2) ."-" .number_format($y_rd/1000, 2);
}
...
function WGS84ToTM35FIN( &$parser, $phi_d, $lambda_d) {
...
function WGS84ToMTM( &$parser, $phi_d, $lambda_d, $zone='') {
...
//      $Airy1830['f']  = 1/299.3249646;
        ...
        $point['height'] = $height;
        $point = datumtransformation($point, $WGS84, $WGS84toOSGB36, $Airy1830);
        $grid = LatLongToOSGrid($point,$Airy1830,$originOSGB36);
//      level 1 transformation
//      $grid = LatLongToOSGrid($point,$WGS84,$originOSGB36);
//      $grid['E'] += 49;
//      $grid['N'] -= 23.4;
        return gridrefNumToLetGB($grid, 8);
}
...
//      $Airy1830_m['f']= ;
        ...
        $point['lambda'] = deg2rad($lambda_d);
        $point['height'] = $height;
//      $point = datumtransformation($point, $WGS84, $WGS84toOSI, $Airy1830_m);
//      $grid = LatLongToOSGrid($point,$Airy1830_m,$originOSI);
//      level 1 transformation
        $grid = LatLongToOSGrid($point,$WGS84,$originOSI);
        $grid['E'] += 49;
        $grid['N'] -= 23.4;
        return gridrefNumToLetIG($grid, 8);
}
...
// WGS84 Latitude/Longitude to ITM Ireland (doesn't work yet !!!!!!!!!!!!!!)
        ...
        $point['phi'] = deg2rad($phi_d);
        $point['lambda'] = deg2rad($lambda_d);
        $grid = LatLongToOSGrid($point,$WGS84,$originITM);
...
//      no datumtransformation needed, Luxembourg is small enough
        $point['phi'] = deg2rad($phi_d); 
        $point['lambda'] = deg2rad($lambda_d); 
        $grid = LatLongToOSGrid($point,$HAYFORD24,$originLUREF);
        extract($grid);
        return sprintf("%dm E %dm N",$E, $N);
 
}
 
function datumtransformation($point, $e1, $t, $e2) {
// Based on http://www.movable-type.co.uk/scripts/latlong-convert-coords.html
